import React from "react";

const TeamContent = () => {
  return (
    <div className="flex flex-col mt-8 md:mt-12 lg:mt-20">
      <div className="container w-full px-4 mx-auto">
        <div className="flex flex-wrap justify-center text-center mb-16 ">
          <div className="w-full lg:w-6/12 px-4">
            <h1 className="text-gray-900 text-xl md:text-3xl lg:text-4xl font-bold mb-4">
              Meet our Team
            </h1>
            <p className="text-gray-700 text-lg md:text-xl  font-light">
              This wonderful people make it all possible👏👏
            </p>
          </div>
        </div>

        <div className="flex flex-wrap">
          <div className="w-full md:w-6/12 lg:w-3/12 mb-10 px-6 sm:px-6 lg:px-4">
            <div className="flex flex-col">
              <a href="/">
                <img
                  className="rounded-2xl drop-shadow-md hover:drop-shadow-xl transition-all duration-200 delay-100"
                  src="https://media.istockphoto.com/photos/portrait-of-confident-young-medical-doctor-on-blue-background-picture-id1161336374?k=20&m=1161336374&s=612x612&w=0&h=ER2Gs06udn4kBPUy8SwLQR2su0GsRWe0kRHZvDbcQCc="
                  alt="team member 1"
                />
              </a>
              <div className="text-center mt-6">
                <h1 className="text-gray-900 text-xl font-bold mb-1">
                  Dr. Tony Stark
                </h1>
                <div className="text-gray-700 font-light mb-2">
                  Founder & Specialist
                </div>
              </div>
            </div>
          </div>

          <div className="w-full md:w-6/12 lg:w-3/12 mb-10 px-6 sm:px-6 lg:px-4">
            <div className="flex flex-col">
              <a href="/">
                <img
                  className="rounded-2xl drop-shadow-md hover:drop-shadow-xl transition-all duration-200 delay-100"
                  src="https://media.istockphoto.com/photos/male-medical-professional-is-confident-in-studio-picture-id1201439096?k=20&m=1201439096&s=170667a&w=0&h=aYuJxV4SkvfMRjuacN_UeOn4t4aixa08wpPZxvBa82w="
                  alt="team member 2"
                />
              </a>

              <div className="text-center mt-6">
                <h1 className="text-gray-900 text-xl font-bold mb-1">
                  Dr. Bruce Banner
                </h1>
                <div className="text-gray-700 font-light mb-2">
                  Dental Specialist
                </div>
              </div>
            </div>
          </div>

          <div className="w-full md:w-6/12 lg:w-3/12 mb-10 px-6 sm:px-6 lg:px-4">
            <div className="flex flex-col">
              <a href="/">
                <img
                  className="rounded-2xl drop-shadow-md hover:drop-shadow-xl transition-all duration-200 delay-100"
                  src="https://thumbs.dreamstime.com/b/indian-male-doctor-clipboard-stethoscope-medicine-profession-healthcare-concept-smiling-white-coat-over-grey-138207484.jpg"
                  alt="team member 3"
                />
              </a>
              <div className="text-center mt-6">
                <h1 className="text-gray-900 text-xl font-bold mb-1">
                  Dr. Bruce Wayne
                </h1>
                <div className="text-gray-700 font-light mb-2">
                  Neuro Specialist
                </div>
              </div>
            </div>
          </div>

          <div className="w-full md:w-6/12 lg:w-3/12 mb-10 px-6 sm:px-6 lg:px-4">
            <div className="flex flex-col">
              <img
                className="rounded-2xl drop-shadow-md hover:drop-shadow-xl transition-all duration-200 delay-100"
                src="https://thumbs.dreamstime.com/b/smiling-medical-worker-white-coat-tie-portrait-doctor-posing-against-gray-background-185380145.jpg"
                alt="team member 4"
              />
              <div className="text-center mt-6">
                <h1 className="text-gray-900 text-xl font-bold mb-1">
                  Dr. Steve Rogers
                </h1>
                <div className="text-gray-700 font-light mb-2">
                  Social Media Manager
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default TeamContent;
